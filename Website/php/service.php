<?php
	...
	if($action == "get_events") {
		...
		logEvent($db_handler, TYPE_CFG_CHANGE, "Impostato ON periodo per la valvola $valve_id: $start_time -> $end_time");
		...
		$start_time = date("H:i", $start_ts);
		$end_time = date("H:i", $end_ts);
		logEvent($db_handler, TYPE_CFG_CHANGE, "Aggiornato ON periodo per la valvola $valve_id: $start_time -> $end_time");		
		...
		$start_time = date("H:i", $start_ts);
		$end_time = date("H:i", $end_ts);
		logEvent($db_handler, TYPE_CFG_CHANGE, "Eliminato ON periodo per la valvola $valve_id: $start_time -> $end_time");		
				...
				if($managed_by != MANAGED_BY_SWITCH) {
					...
					if($status_id == 0) {
						...
						if(strpos($response, "OK") !== false) {
							...
							logEvent($db_handler, TYPE_OUT_OFF, "Valvola $output_id impostata manualmente OFF via web");
							logMessage("IrrighinoService - OUT_ID $output_id turned OFF and event logged in the DB");
							sendReturnCode(0, "OK");							
						}
						else {
							logEvent($db_handler, TYPE_ERROR, "Impossibile impostare manualmente ad OFF la valvola $output_id");
					...
					// new status ON
						...
						if(strpos($response, "OK") !== false) {
							...
							// log the event and return OK
							logEvent($db_handler, TYPE_OUT_ON, "Valvola $output_id impostata manualmente ON via web");
							logMessage("IrrighinoService - OUT_ID $output_id turned ON and event logged in the DB");
							sendReturnCode(0, "OK");							
						}
						else {
							logEvent($db_handler, TYPE_ERROR, "Impossibile impostare manualmente ad ON la valvola $output_id");
							logMessage("IrrighinoService - Error while calling Arduino to turn ON OUT_ID $output_id, response: $response");
					...
					// new status AUTO, just update the database
						...
						// log the event and return OK
						logEvent($db_handler, TYPE_MANUAL_OVERRIDE, "Valvola $output_id impostata manualmente AUTO via web");
